...
from .swin_arch import BasicLayer
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        #         nn.Conv2d(model_channels, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
        # self.conv_up1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        # self.conv_up2 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        # self.conv_hr = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        # self.conv_last = nn.Conv2d(num_feat, out_channels, 3, 1, 1)
        # self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.conv_before_upsample = nn.Sequential(nn.Conv2d(model_channels, num_feat, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True))
        self.pixelshuffleupsample = nn.Sequential(nn.Conv2d(num_feat, 25 * num_feat, 3, 1, 1),
                                      nn.PixelShuffle(5))
        self.conv_last = nn.Conv2d(num_feat, out_channels, 3, 1, 1)
    def forward(self, x, timesteps, un=None, lq=None):
        ...
        out = self.conv_last(self.pixelshuffleupsample(h))
        return out
